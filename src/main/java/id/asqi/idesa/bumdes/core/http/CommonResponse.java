...
//		response.setValidation(Constants.validateErrorMessage(errors));
	...
	public static <T> ResponseEntity<Response<T>> validationError (Map<String, String> validationErrorMessages) {
		Response<T> response = new Response<>(); response.setResponseCode(ResponseCode.VALIDATION);
		response.setValidation(validationErrorMessages); return ResponseEntity.ok(response);
	...
	public static <T> ResponseEntity<Response<T>> noData () {
		Response<T> r = new Response<>(); r.setResponseCode(ResponseCode.NO_DATA); return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> noData (String message) {
		Response<T> r = new Response<>(); r.setResponseCode(ResponseCode.NO_DATA); r.setMessage(message);
		return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> unauthenticated () {
		Response<T> r = new Response<>();
		r.setCode(401); r.setMessage("Unauthenticated"); return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> unauthenticated (String message) {
		Response<T> r = new Response<>(); r.setCode(401); r.setMessage("Unauthenticated: " + message);
		return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> forbidden () {
		Response<T> r = new Response<>(); r.setResponseCode(ResponseCode.FORBIDDEN); return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> forbidden (String message) {
		Response<T> r = new Response<>(); r.setResponseCode(ResponseCode.FORBIDDEN); r.setMessage(message);
		return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> success () {
		Response<T> r = new Response<>(); r.setResponseCode(ResponseCode.SUCCESS); return ResponseEntity.ok(r);
	}

	public static ResponseEntity<Response<Void>> stored (String entityName) {
		Response<Void> r = new Response<>(); r.setResponseCode(ResponseCode.SUCCESS);
		r.setMessage(entityName.concat(" berhasil disimpan")); return ResponseEntity.ok(r);
	}
	...
	public static ResponseEntity<Response<Void>> updated (String entityName) {
		Response<Void> r = new Response<>(); r.setResponseCode(ResponseCode.SUCCESS);
		r.setMessage(entityName.concat(" berhasil diperbarui")); return ResponseEntity.ok(r);
	}
	...
	public static ResponseEntity<Response<Void>> softDeleted (String entityName) {
		Response<Void> r = new Response<>(); r.setResponseCode(ResponseCode.SUCCESS);
		r.setMessage(entityName.concat(" berhasil dihapus (soft delete)")); return ResponseEntity.ok(r);
	}
	...
	public static ResponseEntity<Response<Void>> success (String message) {
		Response<Void> r = new Response<>(); r.setResponseCode(ResponseCode.SUCCESS); r.setMessage(message);
		return ResponseEntity.ok(r);
	}

	public static ResponseEntity<Response<Void>> restored (String entityName) {
		Response<Void> r = new Response<>(); r.setResponseCode(ResponseCode.SUCCESS);
		r.setMessage(entityName.concat(" berhasil direstore")); return ResponseEntity.ok(r);
	}
	...
	public static <T> ResponseEntity<Response<T>> serverError () {
		Response<T> r = new Response<>(); r.setResponseCode(ResponseCode.SERVER_ERROR); return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> databaseError (Exception e) {
		Response<T> r = new Response<>(); r.setResponseCode(ResponseCode.DATABASE_ERROR); r.setMessage(e.getMessage());
		return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> data (T data) {
		Response<T> r = new Response<>(); r.setResponseCode(ResponseCode.SUCCESS); r.setData(data);
		return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> data (T data, String message) {
		Response<T> r = new Response<>(); r.setResponseCode(ResponseCode.SUCCESS); r.setData(data);
		r.setMessage(message); return ResponseEntity.ok(r);
	}

	public static <T, D> ResponseEntity<Response<Page<T>>> paginated (Page<T> page, List<D> dtoList) {
		Response<Page<T>> r = new Response<>(); r.setResponseCode(ResponseCode.SUCCESS); r.setData(page, dtoList);
		return ResponseEntity.ok(r);
	}

	public static <P> ResponseEntity<Response<Page<P>>> paginated (Page<P> page) {
		Response<Page<P>> r = new Response<>(); r.setResponseCode(ResponseCode.SUCCESS); r.setPageData(page);
		return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> badRequest (String message) {
		Response<T> r = new Response<>(); r.setCode(400); r.setMessage(message); return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> noEndpoint (String message) {

		Response<T> r = new Response<>(); r.setCode(404); r.setResponseCode(ResponseCode.NO_END_POINT);
		r.setMessage(message); return new ResponseEntity<>(r, HttpStatus.NOT_FOUND);
	}

	public static ResponseEntity<Response<AuthResponse>> authResponse (AuthResponse authResponse) {
		Response<AuthResponse> r = new Response<>(); r.setResponseCode(ResponseCode.SUCCESS); r.setData(authResponse);
		r.setMessage("authentication success"); return ResponseEntity.ok(r);
	}

	public static <T> ResponseEntity<Response<T>> logoutResponse (ResponseCookie jwtCookie, ResponseCookie refreshTokenCookie) {
		Response<T> r = new Response<>(); r.setMessage("logout success");
	...
	public static <T> ResponseEntity<Response<T>> conflict (String message) {
		Response<T> r = new Response<>(); r.setCode(409); r.setMessage(message); return ResponseEntity.ok(r);
	}
